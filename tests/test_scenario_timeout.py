...
def test_our_timeout(rcst_comm: Communication):
    ...
    rcst_comm.change_referee_command('TIMEOUT_BLUE', 2.0)  # Robots should stop
...
def test_theirr_timeout(rcst_comm: Communication):
    rcst_comm.send_empty_world()
    rcst_comm.send_ball(0, 0)
    for i in range(11):
        rcst_comm.send_blue_robot(i, -1.0, 3.0 - i * 0.5, math.radians(0))
    ...
    rcst_comm.change_referee_command('STOP', 3.0)
    rcst_comm.change_referee_command('TIMEOUT_YELLOW', 2.0)  # Robots should stop
