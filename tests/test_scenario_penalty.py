...
def test_robots_be_behind_ball(rcst_comm: Communication):
    ...
    def blue_robot_are_behind_ball(ball: Ball, blue_robots: RobotDict, yellow_robots: RobotDict) -> bool:
        DISTANCE = 1.0  # meters

        kicker_counts = 0  # ボールに接近したロボットの数
    ...
    rcst_comm.observer.customized().register_sticky_true_callback("test", blue_robot_are_behind_ball)
...
def test_goalie_on_the_goal_line(rcst_comm: Communication):
    ...
    def goalie_on_the_goal_line(ball: Ball, blue_robots: RobotDict, yellow_robots: RobotDict) -> bool:
        GOALIE_ID = 0
        ...
        for robot in blue_robots.values():
            ...
            if abs(robot.x - HALF_FIELD_LENGTH) < ROBOT_RADIUS and abs(robot.y) < HALF_GOAL_WIDTH:
                return True
        return False

    rcst_comm.observer.customized().register_sticky_true_callback("test", goalie_on_the_goal_line)
