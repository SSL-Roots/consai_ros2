...
def eval_easy_to_pass_radius(enemy_robot_pos, ball_pos, ally_robot_pos):
    ...
    after_sort_enemy_robot = forward_robot(enemy_robot_pos, ball_pos)

    # ボールから味方のロボットまでの距離を計測し，近い順にソートする
    dist_ally_robot = sort_ball_from_ally_robot_distance(
...
def dynamic_enemy_for_pass_to(now_enemy_robot_pos, enemy_robot_vvector, ball_pos, ally_robot_pos):
    ...
    after_sort_enemy_robot = dynamicforward_robot(
        enemy_robot_state, ball_pos, enemy_robot_vvector)

    after_sort_ally_robot = forward_robot(ally_robot_pos, ball_pos,)
    ...
    enemy_robot_cnt = len(after_sort_enemy_robot)
    print("敵ゴール側の敵ロボットの数：", enemy_robot_cnt)
...
def forward_robot(enemy_robot_pos, ball_pos):
...
def dynamicforward_robot(robot_state, ball_pos, robot_vvector):
    ...
    print("パスロボットよりもゴール側にいる敵ロボットの位置座標と速度ベクトル: \n", forward_robot_pos)
    forward_robot_pos = np.array(forward_robot_pos)
    return forward_robot_pos


def sort_ball_from_ally_robot_distance(ball_pos, ally_robot_pos):
...
def forward_position_random_generation(how_many: int):
    generating_pos = []
    if how_many > 1:
        while len(generating_pos) < how_many:
