...
class PassShootObserver:
    ...
    def _search_forward_robots(
            self, pos: State2D, search_offset=0.0, search_our_robots=True,
            exclude_id=-1, our_goalie_id=-1) -> list[int]:
        # 指定した座標より前にいるロボットIDのリストを返す関数

        def search(pos: State2D, robots: dict[int, PosVel], search_offset=0.0) -> list[int]:
            # 指定した座標より前にいるロボットIDのリストを返す関数
            forward_robots_id = []
            for robot_id, robot in robots.items():
                if robot_id == exclude_id or robot_id == our_goalie_id:
                    continue
                if pos.x < robot.pos().x + search_offset:
                    forward_robots_id.append(robot_id)

            return forward_robots_id

        if search_our_robots:
            return search(pos, self._our_robots, search_offset)
        else:
