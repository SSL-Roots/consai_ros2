...
class Zone2Decision(ZoneDefenseDecisionBase):

    def __init__(self, robot_operator, field_observer):
        super().__init__(robot_operator, field_observer)

    def stop(self, robot_id):
        self._zone_defense(robot_id, self.ACT_ID_STOP, ZoneDefenseID.ZONE2, without_mark=True)

    def inplay(self, robot_id):
        self._zone_defense(robot_id, self.ACT_ID_INPLAY, ZoneDefenseID.ZONE2)

    def our_pre_kickoff(self, robot_id):
        self._zone_defense(robot_id, self.ACT_ID_PRE_KICKOFF, ZoneDefenseID.ZONE2, without_mark=True)

    def our_kickoff(self, robot_id):
        self._zone_defense(robot_id, self.ACT_ID_KICKOFF, ZoneDefenseID.ZONE2, without_mark=True)
    ...
    def our_direct(self, robot_id):
        self._zone_defense(robot_id, self.ACT_ID_DIRECT, ZoneDefenseID.ZONE2, without_mark=False)

    def their_direct(self, robot_id):
        self._zone_defense(robot_id, self.ACT_ID_DIRECT, ZoneDefenseID.ZONE2, without_mark=False)

    def our_indirect(self, robot_id):
        self._zone_defense(robot_id, self.ACT_ID_INDIRECT, ZoneDefenseID.ZONE2)
