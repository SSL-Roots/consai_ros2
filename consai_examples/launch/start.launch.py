...
def generate_launch_description():
    ...
    declare_arg_game = DeclareLaunchArgument(
        "game", default_value="false", description=('Set "true" to run game script.')
    )

    declare_arg_goalie = DeclareLaunchArgument(
        "goalie", default_value="0", description=("Set goalie id for game script.")
    )
    ...
    declare_arg_referee_addr = DeclareLaunchArgument(
        "referee_addr", default_value="224.5.23.1", description=("Set multicast address to connect Game Controller.")
    )

    declare_arg_referee_port = DeclareLaunchArgument(
        "referee_port", default_value="10003", description=("Set multicast port to connect Game Controller.")
    )
    ...
    param_publisher = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [get_package_share_directory("consai_description"), "/launch/parameter_publisher.launch.py"]
        )
    )

    controller = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [get_package_share_directory("consai_robot_controller"), "/launch/controller.launch.py"]
        ),
        launch_arguments={
    ...
    game_node = Node(
        ...
        arguments=[
            ...
            "--goalie",
            LaunchConfiguration("goalie"),
        ],
        condition=IfCondition(LaunchConfiguration("game")),
        on_exit=Shutdown(),
    )

    set_colorized_output = launch.actions.SetEnvironmentVariable(name="RCUTILS_COLORIZED_OUTPUT", value="1")
