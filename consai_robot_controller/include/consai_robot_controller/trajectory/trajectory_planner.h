...
private:
    ...
    std::vector<BangBangTrajectory2D> generate_trajectories(Vector2D p0, Vector2D v0, Vector2D pg)
    {
        ...
        for (double angle = look_ahead_angle; angle < look_ahead_angle + this->sample_angle_range; angle += this->sample_angle_step)
        {
            // 目的地を生成
    ...
    BangBangTrajectory2D get_best_trajectory(std::vector<BangBangTrajectory2D> &trajectories, Vector2D pg, std::vector<IObstacle *> &obstacles, double t = 0)
    {
        ...
        std::sort(eval_traj_pairs.begin(), eval_traj_pairs.end(), [](const auto &x, const auto &y)
    ...
    std::pair<double, double> _get_obstacle_evals(BangBangTrajectory2D traj, std::vector<IObstacle *> &obstacles, double t_from_start)
    {
        // 障害物に衝突するまでの時間
        ...
        NonRotatingRectangle traj_area(traj_min_position.x, traj_min_position.y, traj_area_width, traj_area_height);
        std::vector<IObstacle *> filtered_obstacles;
        for (IObstacle *obstacle : obstacles)
        {
...
public:
    ...
    BangBangTrajectory2D planOnce(Vector2D p0, Vector2D v0, Vector2D pg, std::vector<IObstacle *> obstacles)
    {
