...
Vector2D >> get_intersection_point_circle_lineseg(Circle circle, LineSegment seg) {
  ...
  if (l.x * p2_temp.x + l.y * p2_temp.y < 0 ||
    sqrt(p2_temp.x * p2_temp.x + p2_temp.y * p2_temp.y) > sqrt(l.x * l.x + l.y * l.y))
  {
    p2 = {false, Vector2D()};
  } else {
...
bool does_collide_non_rotating_rectangle_circle(
  const NonRotatingRectangle & rect,
  const Circle & circle)
{
...
bool does_collide_non_rotating_rectangle_non_rotating_rectangle(
  const NonRotatingRectangle & rect1,
  const NonRotatingRectangle & rect2)
{
  // Checking in the x-direction
