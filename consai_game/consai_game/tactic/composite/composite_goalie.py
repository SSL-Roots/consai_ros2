...
class CompositeGoalie(CompositeTacticBase):
    ...
    def __init__(self):
        ...
        super().__init__(
            tactic_positioning=GoaliePositioning(),
            tactic_defend_goal=DefendGoal(),
            tactic_ball_clear=BallClear(),
        )
    ...
    def run(self, world_model: WorldModel) -> MotionCommand:
        ...
        if self._is_likely_to_score(field, field_points, ball, ball_activity):
            # ボールがゴールに入りそうならブロック
            return self.tactic_defend_goal.run(world_model)
        elif (
            ...
            return self.tactic_ball_clear.run(world_model)
        else:
            # ゴーリーのポジショニングを実行
            return self.tactic_positioning.run(world_model)
